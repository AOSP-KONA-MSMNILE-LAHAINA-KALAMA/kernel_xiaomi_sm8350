...
int panel_index = MI_DISP_PRIMARY;
...
void mi_dsi_panel_state_count(struct dsi_panel *panel, PANEL_COUNT_EVENT event, int value)
{
	...
	if (!strcmp(panel->type, "primary")) {
		panel_index = MI_DISP_PRIMARY;
		strcpy(HWMON_CONPONENT_NAME, "display");
	} else {
		panel_index = MI_DISP_SECONDARY;
	...
	case PANEL_ACTIVE:
	{
		...
		if (value && !panel->mi_count.panel_active_count_enable) {
			timestamp_panelon[panel_index] = get_jiffies_64();
			on_times[panel_index]++;
			...
			off_times[panel_index]++;
			pr_info("%s: %s on_times[%llu] off_times[%llu]\n", __func__, HWMON_CONPONENT_NAME, on_times[panel_index], off_times[panel_index]);

			jiffies_time = get_jiffies_64();
			if (time_after64(jiffies_time, timestamp_panelon[panel_index]))
				panel->mi_count.panel_active += (jiffies_time - timestamp_panelon[panel_index]);
	...
	case PANEL_BACKLIGHT:
	{
		...
		if (last_bl_start[panel_index] == 0) {
			last_bl_lvl[panel_index] = value;
			last_bl_start[panel_index] = bl_level_end;
			return;
		}

		if (last_bl_lvl[panel_index] > 0) {
			panel->mi_count.bl_level_integral += last_bl_lvl[panel_index] * (bl_level_end - last_bl_start[panel_index]);
			panel->mi_count.bl_duration += (bl_level_end - last_bl_start[panel_index]);
		}

		/* backlight level 2047*0.75 ==> 1535 */
		if (last_bl_lvl[panel_index] > 1535) {
			panel->mi_count.bl_highlevel_duration += (bl_level_end - last_bl_start[panel_index]);
		} else if (last_bl_lvl[panel_index] > 0) {
		/* backlight level (0, 1535] */
		panel->mi_count.bl_lowlevel_duration += (bl_level_end - last_bl_start[panel_index]);
		}
	...
	case PANEL_HBM:
	{
		...
		if (value > panel->mi_cfg.hbm_backlight_threshold && enter_flag[panel_index]) {
			time_hbm_on[panel_index] = get_jiffies_64();
			enter_flag[panel_index] = 0;
			exit_flag[panel_index] = 1;
		} else if (value <= panel->mi_cfg.hbm_backlight_threshold && exit_flag[panel_index]) {
			time_hbm_off = get_jiffies_64();
			if (time_after64(time_hbm_off, time_hbm_on[panel_index])) {
				panel->mi_count.hbm_duration += (time_hbm_off-time_hbm_on[panel_index]);
				panel->mi_count.hbm_times++;
			}
			enter_flag[panel_index] = 1;
			exit_flag[panel_index] = 0;
	...
	case PANEL_FPS:
	{
		...
		if (first_into_flag[panel_index]) {
			timestamp_fps[panel_index] = get_jiffies_64();
			first_into_flag[panel_index] = false;
			cur_fps_index[panel_index] = change_fps_index;
			return;
		}

		if (change_fps_index != cur_fps_index[panel_index]) {
			jiffies_time = get_jiffies_64();
			if (cur_fps_index[panel_index] != FPS_MAX_NUM) {
				if (time_after64(jiffies_time, timestamp_fps[panel_index]))
					panel->mi_count.fps_times[cur_fps_index[panel_index]] += (jiffies_time - timestamp_fps[panel_index]);
				snprintf(ch, sizeof(ch), "%llu", panel->mi_count.fps_times[cur_fps_index[panel_index]] / HZ);
				update_hw_monitor_info(HWMON_CONPONENT_NAME, hwmon_key_fps[cur_fps_index[panel_index]], ch);
			}
			timestamp_fps[panel_index] = get_jiffies_64();
		}
		cur_fps_index[panel_index] = change_fps_index;
...
void mi_dsi_panel_HBM_count(struct dsi_panel *panel, int enable, int off)
{
	...
	if (off) {
		if (last_HBM_status[panel_index] == 1)
			record = true;
	} else {
		if (enable) {
			if (!last_HBM_status[panel_index]) {
				/* get HBM on timestamp */
				timestamp_hbmon[panel_index] = get_jiffies_64();
			}
		} else if (last_HBM_status[panel_index]){
			record = true;
		}
	}
	...
	if (record) {
		...
		jiffies_time = get_jiffies_64();
		if (time_after64(jiffies_time, timestamp_hbmon[panel_index]))
			panel->mi_count.hbm_duration += (jiffies_time - timestamp_hbmon[panel_index]) / HZ;
	...
	last_HBM_status[panel_index] = enable;
...
void mi_dsi_panel_HBM_count_bl(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if(bl_lvl > panel->mi_cfg.hbm_backlight_threshold && enter_flag[panel_index]) {
		time_hbm_on[panel_index] = get_jiffies_64();
		enter_flag[panel_index] = 0;
		exit_flag[panel_index] = 1;
	} else if (bl_lvl <= panel->mi_cfg.hbm_backlight_threshold && exit_flag[panel_index]) {
		time_hbm_off[panel_index] = get_jiffies_64();
		if (time_after64(time_hbm_off[panel_index], time_hbm_on[panel_index])) {
			panel->mi_count.hbm_duration += (time_hbm_off[panel_index] - time_hbm_on[panel_index]) / HZ;
			panel->mi_count.hbm_times++;
			enter_flag[panel_index] = 1;
			exit_flag[panel_index] = 0;
		}
...
void mi_dsi_panel_backlight_count(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (last_bl_start[panel_index] == 0) {
		last_bl_level[panel_index] = bl_lvl;
		last_bl_start[panel_index] = bl_level_end;
		return;
	}

	if (last_bl_level[panel_index] > 0) {
		panel->mi_count.bl_level_integral += last_bl_level[panel_index] * (bl_level_end - last_bl_start[panel_index]);
		panel->mi_count.bl_duration += (bl_level_end - last_bl_start[panel_index]);
	}

	/* backlight level 2047*0.75 ==> 1535 */
	if (last_bl_level[panel_index] > 1535) {
		panel->mi_count.bl_highlevel_duration += (bl_level_end - last_bl_start[panel_index]);
	} else if (last_bl_level[panel_index] > 0) {
		/* backlight level (0, 1535] */
		panel->mi_count.bl_lowlevel_duration += (bl_level_end - last_bl_start[panel_index]);
	}
...
void mi_dsi_panel_fps_count(struct dsi_panel *panel, u32 fps, u32 enable)
{
	...
	if (!panel || (!enable && timming_index[panel_index] == FPS_MAX_NUM))
		return;

	if (enable && !fps)
		fps = cur_fps[panel_index];
	...
	if (i < FPS_MAX_NUM || !fps) {
		if (i != timming_index[panel_index] && timming_index[panel_index] < FPS_MAX_NUM) {
			jiffies_time = get_jiffies_64();
			if (time_after64(jiffies_time, timestamp_fps[panel_index]))
				panel->mi_count.fps_times[timming_index[panel_index]] += (jiffies_time - timestamp_fps[panel_index]) / HZ;
			snprintf(ch, sizeof(ch), "%llu", panel->mi_count.fps_times[timming_index[panel_index]]);
			update_hw_monitor_info(HWMON_CONPONENT_NAME, hwmon_key_fps[timming_index[panel_index]], ch);
			timestamp_fps[panel_index] = jiffies_time;
		} else if (timming_index[panel_index] == FPS_MAX_NUM)
			timestamp_fps[panel_index] = get_jiffies_64();

		timming_index[panel_index] = i;
	}

	cur_fps[panel_index] = fps ? fps : cur_fps[panel_index];
}
